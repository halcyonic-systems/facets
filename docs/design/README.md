...
- [`neutral-interchange.md`](neutral-interchange.md) — RESEARCH SEED (2026-07-24): the neutral interchange as a *serialization of the kernel* `(T, R)` rather than "CSV for a domain", with lenses doing the domain interpretation on ingest. Framing to design against, not a build spec.

## Plans and drafts (execution records, not positions)

These carry no normative weight; they exist so an implementation session is execution rather than re-derivation. A plan that has shipped stays here as the record of what was planned.
