...
| **Missing** *(honesty)* | No reachability, dead-end, or duplicate-*edge* check exists (`check_duplicate_ids` catches duplicate *ids* only). Filed as **#66**. Two of the bill.json errors were caught by hand only because these checks don't exist yet. | Absent — a known gap, not a guarantee. |
