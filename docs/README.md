...
- [`288-level-census-draft.md`](288-level-census-draft.md) — **RESEARCH** — the [#288](https://github.com/halcyonic-systems/bert-lenses/issues/288) Klir epistemological-level census over all 34 corpus + example entries: four levels named in prose, ~31 proposed at structure, and an eight-row FORK ledger awaiting ratification — five of the eight forks being one cross-cutting question (does engine-run dynamics over declared quantities count as an authored generating rule?). **A judgment draft: nothing is declared in-file except the three prose-named entries.**
