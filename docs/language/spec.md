...
- **Δt is currently a semantic tick, not a numerical step — and the unit strings overstate (#216-adjacent finding, 2026-07-27).** `amount` declares units per TIME (`ML/mo`, `kW`) but the engine consumes it per TICK: `RecordedRun` stores `dt` as metadata the dynamics never read, so the same file run over the same horizon at Δt = 1.0 and Δt = 0.5 produces totals a factor of ≈2 apart. The rate reading — the one the units and the `(T, Δt)` form already promise — is asserted by `crates/bert-canvas/tests/dt_invariance.rs`, standing red until the engine scales flux by Δt (or the unit claims are withdrawn). Until then, treat every stated per-time unit as per-tick.
