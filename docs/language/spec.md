# SL — The bert-lenses System Language, v1.6

**Status: LIVE.** Normative, and specified from the working implementation (2026-07-18). Parser and serializer: `crates/bert-canvas/src/sl.rs`. Boundary: `compile_sl` / `emit_sl` (`crates/bert-lenses-kernel/API.md`, "SL surface"). Round-trip goldens: `crates/bert-canvas/tests/sl_roundtrip.rs` over `fixtures/sl/`; formatting goldens: `crates/bert-canvas/tests/sl_format.rs` over `fixtures/sl/stanza/`. Design rationale and precedent research: `docs/design/sl2-authoring-language.md`. Issue: #82.

SL is a human-writable textual notation that compiles deterministically into a bert-lenses model. It is the third concrete syntax over the one neutral abstract spec the repo already holds — the canvas authors it by gesture, JSON serializes it, SL writes it as text. This document specifies the v1 language as it stands at v1.7 — v1.0 (2026-07-18) plus the additive revisions since, each marked `(vX.Y, #issue)` where it lands and rowed in §11: its lexicon, grammar, semantics, the annotation layer, the round-trip contract, and the structure/dynamics boundary. Three worked examples (the golden corpus) and a lineage section close it.
...
**C4 — Presentation never contaminates meaning.** Structure lines (§4) carry model content; `@`-prefixed annotation lines (§6) carry view state — positions, the active lens, an observer's direction toggle. The parser applies annotations but no systemhood-bearing fact depends on them; a diff of the structure lines shows systems changing, never node drags. One file holds both, annotations last (the Modelica precedent: layout is syntactically present, semantically null). Within the structure lines a third, **authorial** tier is named as of v1.7 (facets#411): `description` and `grounding` ride structure lines, project into `Info`, and round-trip, but no verdict reads them — two models differing only there are the same system. They are neither systemhood nor view; they are what the author says about the claim. `description` had these properties from v1.4 without a name for them.
...
| `description` | prose about a thing or a flow, in the author's own words — restored in v1.4 (#326) after the rebuild dropped what the original BERT carried on every entity. **Never semantics**: no verdict reads it, and two models differing only here are the same system. It projects into `Info.description`, a bert-core field that existed from the start and received an empty string until this clause reached it. Since v1.5 (#399) it may sit on its own indented line directly beneath its declaration — the one clause that can, because it is the one clause whose value is a sentence (§4.3) | house word; the DISTINCTION is old-bert's, and the destination field is bert-core's | `Thing.description` / `Relation.description` → `Info.description` |
...
| `grounding` | whose word this thing or flow rests on (v1.7, facets#411): a grade from a closed, ordered vocabulary — `unknown` < `third-party` < `asserted` < `observed` < `spec` ≈ `chain` — and, optionally, the reference in the author's words (a file and line, a URL, an endpoint and date, a block number). Authorial tier (C4): projects into `Info.grounding`, round-trips, and **no verdict reads it**; if a grade ever needs to bite, that is a new ruling, not an extension. Rides environment lines, as `description` does and for the same reason. Takes the continuation form of §4.3, beneath the description when both are present | house word; the DISTINCTION is the modeler's (if this element is wrong, whose word was wrong) | `Thing.grounding` / `Relation.grounding` → `Info.grounding` |
...
| `capacity` | a Buffering stock's overflow ceiling (#112) — the storage clamp: a bounded tank overflows and the excess dissipates (Mobus Fig 3.17). Buffering-only; ≤ 0 is a parse fault | Mobus (containers have capacity — Ch. 4) | `Thing.cognitive_params["capacity"]` → `AgentModel.cognitive_params["capacity"]` |
| `time constant` | a Buffering stock's first-order drain coefficient τ (#112) — `release ≈ stock/τ`, an exponential decay in place of the fixed-rate drain. Buffering-only; ≤ 0 is a parse fault; MUTUALLY EXCLUSIVE with `release` on one component (the engine prefers time_constant whenever both are set, so a co-declared `release` would never be honored — refused rather than silently overridden). `constant` is positional (§7.1), sitting behind the already-reserved `time` head | Mobus (Fig 4.12's first-order regulator dynamics) | `Thing.cognitive_params["time_constant"]` → `AgentModel.cognitive_params["time_constant"]` |
| `setpoint` | an Inverting comparator's reference value (#112) — `(setpoint − signal).max(0)`, Mobus Fig 4.12; the engine's own default is 1.0 (the bare `1 − signal`). Inverting-only; ≤ 0 is a parse fault | Mobus (Fig 4.12, the comparator) | `Thing.cognitive_params["setpoint"]` → `AgentModel.cognitive_params["setpoint"]` |
| `maintenance` | a Buffering stock's per-tick upkeep loss (#112) — Odum's depreciation outflow, charged from the stock every tick and dissipated (Mobus Fig 3.17: "every store has one"). Buffering-only; ≤ 0 is a parse fault | Mobus (Fig 3.17, Odum depreciation) | `Thing.cognitive_params["maintenance"]` → `AgentModel.cognitive_params["maintenance"]` |
| `backpressure` | a Modulating valve's throttle flag (#112) — a downstream throttled valve holds its release back rather than shedding it. Bare (no value); Modulating-only | Mobus (the throttled-valve regulator) | `Thing.cognitive_params["back_pressure"]` → `AgentModel.cognitive_params["back_pressure"]` |
...
SL is line-oriented. A file is a sequence of lines, each independently one of: blank, comment, structure line, or annotation line — with one exception since v1.5, the indented `description` line (and, since v1.7, the indented `grounding` line), which belongs to the declaration directly above it (§4.3). A blank or comment line stands *above* whichever line comes next — it is that line's leading trivia — and `format_sl` (v1.6, §7.4, #302) is the one thing in this spec that reads a file's blank and comment lines as anything other than noise between structure lines; the compiler itself never does. In EBNF (terminals case-insensitive for keywords; names case-sensitive):

```ebnf
model       = { line } ;
line        = blank | comment | structure [ newline continuation ] | annotation ;
continuation = indent "description" string             (* v1.5, #399: the prose of the thing, iface,  *)
            | indent "grounding" gradeword [ string ] ; (*   milieuvar, or flow on the line directly   *)
indent      = one or more spaces or tabs ;             (*   above; indentation means nothing elsewhere; *)
                                                       (*   grounding: v1.7, #411, not on milieuvar    *)

structure   = system | domain | sysdesc | timeunit | level | thing | iface | milieuvar | flow | param | metric | boundary ;
system      = "system" [ string ] [ ":" kingdom [ "/" genus ] ] ;
domain      = "domain" string ;
sysdesc     = "description" string ;                   (* unindented: the system's own prose, #326 *)
...
iface       = "interface" name [ "protocol" string ]   (* the pass-way as its own object, #226; *)
              ...
              [ "grounding" gradeword [ string ] ] ;   (* #411 *)
...
attr        = "interface" [ "protocol" string ]        (* merged form: this component IS the pass-way *)
            ...
            | "grounding" gradeword [ string ]         (* authorial, #411; rides environment lines *)
            | "stock" name [ "initial" number ]        (* declared stock unit; starting level, #112 *)
            | "release" number                         (* Buffering drain per time unit, #112 *)
            | "capacity" number                        (* Buffering overflow ceiling, #112 *)
            | "time" "constant" number                 (* Buffering drain coefficient τ, #112 *)
            | "setpoint" number                         (* Inverting comparator reference, #112 *)
            | "maintenance" number                      (* Buffering per-tick upkeep loss, #112 *)
            | "backpressure"                            (* Modulating throttle flag, #112 *)
...
flow        = "flow" name "->" name [ ":" kindword ] [ string ]
              ...
              [ "grounding" gradeword [ string ] ] ;    (* last of all, #411 *)
gradeword   = "chain" | "spec" | "observed" | "asserted" | "third-party" | "unknown" ;
...
usabilityword = "Resource" | "Disruption" | "Product" | "Waste" ;
...
An unindented `description "<prose>"` line of its own is the system's description (#326): at most one per file, landing in `CanvasModel.description` beside the name, with the standing of every other description (§4.3) — prose, never semantics. Indented, the same words belong to the declaration above instead (§4.3, v1.5).
...
`component Furnace primitive Combining interface` declares a component, optionally designating it a work process of a named primitive kind and/or a member of the boundary interface set — the merged special case for a component that IS a pass-way. `source` / `sink` / `environment` declare environment things; the attribute words are rejected there (environment internals are opaque — Mobus §4.3.3.2.2, mirrored at `canvas.rs:84`). Names are unique per file; things must be declared before any flow references them. One exception, and it is a fold rather than a duplicate: `source X` followed by `sink X` (or the reverse) declares ONE neighbour that both supplies and receives, and compiles to the same thing `environment X` would — the canonical serializer writes it back as `environment X`. The second line may carry a `description` (kept when the first had none); any other attribute on it is the ordinary duplicate-name fault. Adopted 2026-09-09 (facets#377): the pair was the largest compile fault in every co-author run on both models, and a rule in the prompt did not remove it.
...
`component Battery description "the pack that rides out a cloudy week"` declares what the thing IS, in the author's own words (v1.4, #326). Quoted, at most once per line, and — unlike every other attribute — it **rides environment lines too** (`source Sunlight description "the only energy entering"` is legal): the opacity rule of §4.3.3.2.2 is about an environment thing's INTERNALS, not about naming what it is, and the original BERT's descriptions sit mostly on environment entities. It is prose and never semantics: `validate` never reads it, so two models differing only in their descriptions are the same system and receive the same verdict. It DOES project — into `Info.description`, a bert-core field that has existed from the beginning and received `String::new()` at every construction site until this clause reached it. A `#` comment now round-trips too, through `format_sl` (v1.6, §7.4, #302) — the difference is that a description is *model content*: it projects, and two models differing only in their descriptions are different `CanvasModel`s (equal only by the prose-is-never-semantics rule above), while two models differing only in comments are the same `CanvasModel`, full stop — comments never reach `parse_sl`'s output at all. A second `description` on one line is a fault; an unquoted value is a fault.

`source "GPU hosts" grounding unknown "no Venice document names a host"` says whose word the thing rests on (v1.7, facets#411): a grade, then optionally the reference. The six grades are a closed vocabulary and an order — `unknown` (a drawn hole: nothing found says this, and the author is saying so) < `third-party` (someone else's prose about the subject) < `asserted` (the subject's own prose) < `observed` (the modeler measured it, dated) < `spec` (an artifact that executes) ≈ `chain` (verified source at a stated address and block, or a formal proof) — and the order is a reading aid for the canvas and the inspector, never a rule a verdict applies. Authorial tier (C4): it projects into `Info.grounding`, round-trips, is never read by `validate`, and rides environment lines on the same terms as `description`. At most one per declaration; an unknown grade is a fault; a second on one line is a fault. It takes the continuation form of §4.3 (indented, beneath the description when both are present), and the emitter always writes it that way. Not on `milieu` lines: an ambient variable is not a claim with a source in this sense, and nothing projects it to `Info`. The co-author drafter never writes a grade above `asserted`; `chain`, `spec` and `observed` are the modeler's to write.

**The description may sit beneath its declaration (v1.5, #399).** The same clause, on its own indented line directly under the line it describes:

```
component "Switchgear" interface
    description "Utility service entrance, breakers and automatic transfer switch."
```

The two spellings compile to the same model, and the inline one stays legal forever. The rule is deliberately narrow, because it is the only place in SL where one line depends on another. **Only `description` continues**: it is the one clause whose value is a sentence and the one clause that trails every carrier, so lifting it off the line moves nothing else; `protocol`, `stock`, `unit`, and the rest stay where they are, and a string still cannot span lines (§7.3). **Only an indented line that opens with `description` is a continuation**: any leading space or tab marks it (the amount is not significant), while an *unindented* `description` keeps the meaning it has always had, the system's own (§4.2). Every other indented line reads exactly as it did before v1.5, indentation ignored — an indented `component B` is still a component, and an indented `primitive Combining` is still the unknown-keyword fault it always was — so nothing that parsed before can newly fault. **A wholly indented file is its dedented self**: before any of this, the leading whitespace that *every* non-blank line shares (comments included) is dropped, so SL lifted out of a markdown list or a chat reply parses as if it had never been indented, continuations and the system's `description` alike. The shared indent is compared as an exact string prefix — a tab and four spaces share nothing, so mixed indentation is never guessed at, and only what every line truly has in common goes. Lines are never removed, so faults keep their physical line numbers. **Directly beneath means the very next line**: a blank line or a `#` comment in between breaks the attachment, and the continuation is refused rather than reaching upward past it for something to describe (an indented comment is still only a comment). A trailing `#` comment on the continuation line itself is fine. The carriers are exactly the lines that take the inline clause — `component` / `source` / `sink` / `environment`, `interface`, `milieu`, `flow` — including the second line of a folded `source X` / `sink X` pair, where the prose describes the one neighbour. Faults, each reported on the continuation's own line: nothing directly above; the line above did not parse (the message names that line — a description never attaches to a declaration that was refused); the line above is a kind that takes no description (`system`, `domain`, `boundary`, `param`, `metric`, an annotation, …); a description already given, inline or by an earlier continuation; and anything beside `description "<prose>"` on a continuation line.
...
`component Reservoir primitive Buffering stock ML capacity 10 maintenance 0.2` extends the Buffering vocabulary (#112 slice 2): `capacity <n>` declares the stock's overflow ceiling (the storage clamp reads it — overflow dissipates rather than accumulating past it) and `maintenance <n>` declares its per-tick upkeep loss (charged from the stock every tick, dissipated, never delivered). `component Reservoir primitive Buffering stock ML time constant 3` declares the first-order drain coefficient τ in place of a fixed `release` rate — `time constant` is two tokens, `constant` sitting behind the already-reserved `time` head. All three are Buffering-only and ≤ 0 is a parse fault; `time constant` and `release` on the same component is also a parse fault, because the engine's drain arm prefers `time_constant` whenever both are declared, so the `release` would never be honored. `component Thermostat primitive Inverting setpoint 0.8` declares the comparator's reference value (Inverting-only, ≤ 0 a parse fault; the engine's own default is 1.0, the bare `1 − signal`). `component Valve primitive Modulating backpressure` declares the throttle flag (Modulating-only, bare — there is no magnitude to decline). Each of the five is a fault on any other primitive, on an environment thing, or given twice on one line — the same discipline `release` already follows.
...
`usability <Resource|Disruption|Product|Waste>` declares what this crossing IS to the system (v1.4, #331) — Mobus's 2×2 of direction against value. **Undeclared is not `Resource`**: an author who has not ruled has said nothing, and only `project()` supplies the historical default, exactly as `amount` does. It is read by the kernel: two edges between the same pair with the same substance but different usability are a genuine second channel and no longer warn as a duplicate (`check_duplicate_edges`), which is what the warning's own suggestion had promised since before the clause existed.

`description "<prose>"` trails the whole clause chain on a flow (v1.4, #326), for the same reason it emits last: it is the one clause whose value is a sentence, and keeping it at the end leaves the machine-readable clauses adjacent and scannable. Same standing as the thing-line clause — prose, never semantics, projects into the interaction's `Info.description`, round-trips. It takes the continuation form of §4.3 on the same terms (v1.5, #399): indented, directly beneath the `flow` line, and nothing else beside it. `grounding <grade> ["<reference>"]` trails even the description (v1.7, #411): both are prose-valued, so the two sentences bring up the rear and the machine-readable clauses stay adjacent. Same standing as on a thing line.

**The label is a NAME; the sentence goes in `description` (#335).** `"published measurements"` is a label; *"PCE inflation, payrolls and unemployment — the FOMC steers by measurements of the economy, with their lags and revisions, not by the thing itself"* is a description. This is a convention on authors, **not a parser rule**: a long label is legal SL, the kernel reads nothing from it, and refusing one would be the language enforcing taste. It is stated here because the alternative is measurable. Labels became sentences for a structural reason rather than carelessness — before `description` existed there was nowhere else to put prose, so the label was the only slot — and on `federal-reserve.sl` that produced 23 labels with 7 overlapping pairs, the widest 224px against a 30px parallel-sibling fan; moving the prose took the widest to 83px. Crowding cannot be zoomed away, either: every label sits inside one `scale()` group and nothing counter-scales, so text and separation grow together. Past roughly 28 characters the canvas elides the label and keeps the full text on hover, so a sentence in this slot is not merely cramped — it is not shown. Our own shipped models are held to the budget by a test (`crates/bert-canvas/tests/examples.rs`); `assets/corpus/**` is exempt, because those labels are transcribed from published figures and are their authors' words.
...
Parsing collects **all** faults in one pass and reports each with its 1-indexed line — never a first-error-only bail, and never a guess. Fault classes: lexical (unterminated quote), unknown keyword, malformed clause, duplicate name, undeclared flow endpoint, redeclared singleton (`system`/`domain`/`time unit`/`level`/`boundary`), an unknown level value, attribute on an environment thing, unstamped or malformed decomposition reference, `decomposes` on an interface component, duplicate `decomposes`, duplicate `stock`, an unknown `usability` value (#331), a duplicate or unquoted `description` (#326), the §4.3 continuation faults (v1.5, #399: an indented `description` line carrying anything but its quoted prose, one with no declaration directly above, one beneath a line that failed or that takes no description, one whose declaration already has its description), the `grounding` faults (v1.7, #411: an unknown grade, a duplicate on one line or across the line and its continuation, an indented `grounding` line carrying anything beyond its grade and quoted reference, one with no declaration above or beneath a line that takes none), duplicate or malformed Klir metadata clause (`scale`/`states`/`kind` — an unknown scale or kind value, a malformed state set), out-of-range `@directed`, malformed known annotation, the §4.5 param faults (unresolvable or ambiguous anchor, amountless anchor, degenerate or contradicted range, shares without a >=2 component fanout, duplicate param name), the §4.6 metric faults (unresolvable or ambiguous anchor, share over a single-outflow source, sum into a thing with no inflows, duplicate metric name), the §4.4 ample faults (beside `amount`, with a `unit`, on a `mere` relation, on a non-informational kind), and the §4.3 engine-parameter faults (#112: `release`/`capacity`/`time constant`/`maintenance` off a Buffering component, `setpoint` off an Inverting component, `backpressure` off a Modulating component, a duplicate of any of the six on one line, a non-positive value where one is required, and `release` co-declared with `time constant` on the same component). The ACE/Gherkin discipline — deterministic parse, fail loud on anything ambiguous — is inherited deliberately.
...
([#213](https://github.com/halcyonic-systems/facets/issues/213),
[#223](https://github.com/halcyonic-systems/facets/pull/223),
[#225](https://github.com/halcyonic-systems/facets/issues/225),
[#228](https://github.com/halcyonic-systems/facets/pull/228)) a **flowless
...
`emit_sl` writes a model as (a declaration's `description` and then its `grounding`, when present, follow it as indented continuation lines): `system` / `domain` / `description` / `time unit` / `level` lines; thing lines in `things` order (environment words edge-derived per §5.2 — the emitted word is the kernel's reading); `flow` lines in `relations` order; `param` lines then `metric` lines in declaration order (the input/output twins read together, §4.5–4.6); `boundary` if authored; blank line; `@lens`; `@pos` per thing; `@directed` per directed flow. On a thing line the attributes emit in a fixed order: `primitive`, then `interface`, then `stock` (with its welded `initial`, component lines only), then the six typed engine parameters (#112) — `release`, `capacity`, `time constant`, `setpoint`, `maintenance`, `backpressure`, each present only when its key rides `cognitive_params` — then the Klir metadata — `kind`, then `scale`, then `states` (#154; the one attribute group environment lines also carry), then `decomposes` last (component lines only; the reference, quoted label plus `@`-id). A `description` never emits on the declaration's line: on every carrier — thing, `interface`, `milieu`, `flow` — it emits as the continuation line of §4.3, four spaces then `description "<prose>"`, directly beneath (v1.5, #399). Always, not past some width: the form is then a pure function of the model with no number to tune, the declarations read as a column with the prose under them, and inline input normalizes to it in one pass. (The system's own top-level `description` line is unchanged.) Names emit bare when they read as identifiers and shadow no *reserved* word, quoted otherwise. The reserved set (`RESERVED_WORDS`, `sl.rs`) is §4's position-free keywords; the position-bound words — `param`, `metric`, `ample`, `range`, `shares`, `from`, the metric verb words `share`/`of`/`sum`/`into`, `constant` (sitting behind the `time` clause head), and the `@lens` values — never occupy a slot a name can reach, so a name matching one of them stays bare and still re-parses as itself. That the two lists together are exactly §4's terminals is held by `tests/keyword_parity.rs`. Floats emit via shortest-round-trip decimal representation, so re-parsing recovers them bit-exactly.

`emit_sl` itself never reads a comment, a blank line, or the order an author explained things in — none of that is in `CanvasModel` (§2), so `emit_sl` cannot see it. §7.4 is the separate function that does.

### 7.2 The contract (golden-tested)

Three guarantees, distinguished honestly (`tests/sl_roundtrip.rs`, `tests/sl_format.rs`):
...
- **Authored text: formatting keeps every comment (v1.6, #302).** `format_sl` (§7.4) keeps every comment and blank line a source carries: `format(t)` drops no `#` line; `format(format(t)) = format(t)`; and `parse(format(t)) = parse(t)` as JSON — formatting never changes what a file means, only how it is laid out.

### 7.3 Unrepresentable shapes

`emit_sl` refuses loudly rather than lose information silently: a name or label containing `"` or a newline, and a `system_type` with genus but no kingdom, are errors. A third refusal since #216: a thing carrying the opaque engine-parameter maps (`cognitive_params` / `initial_state`, which the canvas carries so a loaded model's dynamical content survives) cannot be written down without narrowing when it holds anything SL has no production for. §4.3 now types `initial_state["storage"]` and six `cognitive_params` keys (`release_rate`, `capacity`, `time_constant`, `setpoint`, `maintenance`, `back_pressure`, each gated to its one reading primitive, #112) — a bag holding exactly those, correctly gated, emits; anything else still refuses and names the JSON export as the lossless path. This refusal does not become dead code as the typed set grows: `cognitive_params` is a `HashMap` fed by both SL parsing and raw JSON loads (compose's exporter, §8.2), so a loaded model can always carry a key the parser has never heard of, and the check is what stops that key from being silently dropped on emit. Nothing else in `CanvasModel` is outside the language. One asymmetry is accepted knowingly: `primitive`/`interface` on an *environment* thing (expressible in JSON, semantically inert — the kernel ignores both) is dropped on emit rather than round-tripped.

### 7.4 Formatting (v1.6, #302)

*Status: normative (golden-tested, `tests/sl_format.rs`). No grammar change — this section is a contract revision only: it names a second, optional serializer and what it preserves that `emit_sl` does not.*

`format_sl(text) = emit_sl_with(parse(text), layout)`, where `layout` is a surface fact the parser reports alongside the model (`SlParse.layout`, never folded into `CanvasModel` — §7.2's contract for `emit_sl` is unchanged because `emit_sl` reads none of it) and `emit_sl_with(m, None) = emit_sl(m)`, byte for byte. A faulted parse yields no layout; `format_sl` then returns the same faults `parse_sl` would, and writes nothing (gofmt semantics — no partial output on a broken file).

What `layout` carries, and how `format_sl` writes it back:

- **Leading trivia.** The comment and blank lines standing above a declaration travel with it: reordering a file into canonical section order (§7.1) moves the declaration's trivia with it, not the lines that happened to sit above wherever it landed. A blank-line run collapses to one; trailing whitespace is stripped, and so is any indentation before a `#`.
- **Trailing trivia.** A same-line `#` comment on a declaration's own line, or on its `description` continuation line (§4.3), re-emits two spaces after the line's content.
- **Section banners get no syntax of their own.** A `# ── flows ──`-style line is ordinary leading trivia of whatever declaration follows it — §4 gains no new construct to name a banner.
- **`@pos` and `@lens` are authored-only.** Where `emit_sl` (`layout: None`) writes one `@pos` per thing and always writes `@lens`, `format_sl` writes a position only for a thing whose source carried one, and `@lens` only if the source wrote it — formatting a file never invents a pin the author never placed.
- **An unrecognized `@`-annotation line survives verbatim**, rather than being dropped by the ignorable-annotation contract (§6) the way plain `emit_sl` still drops it.
- **Trivia the parser cannot attach to any declaration** — text after the last line it consumed — is written back at the end of the file, after the annotation block.

Two things this section deliberately does not cover, both later work: `reemit_sl(source, model)`, which would apply a *canvas* edit to authored text in place (the `layout` shape above is expected to grow a name-keyed index for that); and a machine-comment convention for text an emitter generates and must not re-capture as authored trivia on the next format pass (the crossings comments of §5, `sl.rs` — currently regenerated fresh, never authored, so no such collision exists yet).
...
| ~~A walk is three files and five gestures to load~~ tooling shipped (no grammar change) | facets#412 (2026-09-19) | One `.sl` file may hold several `system` paragraphs; the app's Open… splits them, resolves a bare `decomposes "Name"` to the paragraph so named (relative to the referrer), has the kernel mint the ids, stamps both ends, saves the children, opens the root. The compiler is untouched: a bare reference in a single paragraph is still the §4.3 fault, and identity still never lives in the text |
| ~~No way to say whose word an element rests on~~ shipped as v1.7 | facets#411 (2026-09-18) | `grounding <grade> ["<ref>"]` on thing, interface and flow lines → `Info.grounding`; authorial tier named in C4; six ordered grades; the Venice model is the separating instance |
| ~~`description` could only ride its declaration's line~~ shipped as v1.5 | #399 (landed 2026-09-17) | An indented `description "<prose>"` directly beneath the declaration; the narrow rule — no other clause continues, no multi-line strings — and canonical emit always uses it. See §3, §4.3, §4.4, §7.1. Comment and stanza preservation stays #302's |
| ~~Comments and blank lines were not preserved on emit~~ stage 1 shipped as v1.6 | #302 (stage 1 landed 2026-09-17, epic #399) | `format_sl` (§7.2, §7.4): a layout side-structure (`SlParse.layout`) the parser reports and `emit_sl_with` consumes; `emit_sl`/`CanvasModel` unchanged. Still open (stage 2+): a wasm export, `reemit_sl(source, model)` for canvas edits made against authored text, an orphan-comment policy for a deleted declaration, the reserved `#~` machine-comment prefix, and a `bert fmt` CLI subcommand |
