...
### Implementation strategy: `E`-as-port (2026-07-24)

The single-SoI/SoS distinction has a concrete implementation, cross-checked against five sources in the categorical open-systems lineage — Myers (*Categorical Systems Theory*), Baez (*Double Categories of Open Systems*, arXiv:2509.22584v2), Willems (*Behavioral Approach*), Fong–Spivak (*Seven Sketches*), and `Span(Graph)` (Di Meglio–Sabadini–Walters). Full read + quotes: `operations/sessions/2026-07-24/references/categorical-environment.md`. **Every one reduces "environment" to the composition interface**; none gives it structure beyond "the other side of an interface." `Span(Graph)` is the strongest witness — *zero* occurrences of "environment," and it *proves* boundaries are optimally bare sets via a universal property (a free feedback category), even for stateful transition systems, rather than assuming it. And the *only* place the tradition reifies "Environment" as a box (Myers's Agent/Environment toy, p.14) does so precisely to declare a composite **closed** — functionally identical to Mobus's first-class `E`.

So the doctrine's implementation: **model `E` as a *port* — a stubbed/degenerate cospan-foot — not as a separate node type.** Mobus's first-class `E` is then the *closed* face of a boundary (single-SoI: "no further outside"); the open port is the *same* boundary kept liquid for composition (SoS). The payoff: **un-stubbing an `E` port simply *is* systems-of-systems composition — no new machinery.** This collapses `E` (Mobus), the environment↔environment seam (#166), and the neutral-interchange's "compose at shared entities" (#172, `docs/design/neutral-interchange.md`) into *one* mechanism: environment is a port; composition is un-stubbing it.

Caveat that travels with the citation: the `Span(Graph)` evidence is sourced via a descendant restatement (Di Meglio–Sabadini–Walters, arXiv:2010.10069), not the primary Katis–Sabadini–Walters 1997 AMAST paper (not in the library / paywalled). Strong triangulation, not a primary-text reading of that one line.
