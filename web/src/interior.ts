...
import { draftSlWithRetry, stampInterfacesFromCrossings, type DraftResult, type DraftStage } from "./coauthor";
import { pendingCrossings } from "./canvas/crossings";

export { pendingCrossings, stampInterfacesFromCrossings };
 ...
 *
 *  One named repair rides on top, `stampInterfacesFromCrossings` (coauthor.ts);
 *  everything else is the compiler's output by identity. Since #399 the draft
 *  loop makes the same repair under the kernel's refusal and returns text that
 *  carries it, so under Mobus this pass finds nothing left to stamp. */
