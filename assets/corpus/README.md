...
| `mobus/human-social-system.sl` | George Mobus | *Systems Science: Theory, Analysis, Modeling, and Design* (2022), Ch. 7 §7.6.3, Fig. 7.17 | The whole human enterprise as one SOI in the Earth, and the only entry whose lesson is an absence: resources in, wastes and heat out, and no product returning anything of value to the environment. | The decomposition of the HSS, which Mobus defers to Ch. 9 — and what Ch. 9 delivers is the interior of the whole HSS (Fig. 9.2's nine subsystems include Governance, a peer of Work Processes here), never this component's, so Work Processes stays atomic. Also the individuation of each aggregated source, sink and interface, which he flags as the next step. |
| `klir/criminal-court.sl` | George Klir | *Facets of Systems Science*, 2nd ed. (2001), Ch. 4, book pp. 77–78, Fig. 4.7 | A structure system over an institutional domain: five elements, ten directed variables, a feedback edge, and Klir's own legality rule (each variable is an output of exactly one element) checkable against the model. | The behaviour of every element — Klir's blocks are "initially source systems, which later become data systems", and a source system has no interior at all, so none of the five phases is decomposed. The ten flows are typed `informational`; that kind is the model's reading, not Klir's, and the file says so. |
...
## The entries that teach by refusal

*Corrected 2026-07-27 (#216). This section previously read "Two entries", and named
`coupling-sigma2` as one of them. That was wrong twice: `sigma2` is a lexicon-pressure
entry rather than a refusal, and the cross-lens matrix now run over every model finds
**six** refusals in the tree, not one. Four are below; the two on the demos are noted at
the end. The count was wrong because nothing had ever read an entry under a lens other
than its own — the ship gate validates each entry under its pinned lens only, so the
lessons here were asserted in prose and executed by nothing until `crates/bert-canvas/
tests/cross_lens.rs` bound them.*

**`bunge/coupling-sigma3.sl` does not travel, and that is the lesson.** Bunge admits the diagonal — a thing may act on itself, and his coupling matrix records it. Mobus forbids it (§4.3 requires `k ≠ o`). So the file is a legal Bunge structure and a **refused** Mobus one. Open it and switch to the Mobus lens: *"self-dependency is not representable in the 8-tuple."* A real divergence between two traditions, shown rather than asserted. Now asserted by a test that fails if the self-actions are removed.

**`mobus/steel-plant.sl` is Mobus's own flagship SL exemplar, and Bunge refuses it.** Fig. 4.14 draws the plant as one opaque box among three sources and three sinks, six flows crossing a boundary and nothing beneath it. Under Mobus that is a system at level −1. Under Bunge it is not a system: Def 1.1's bondage is quantified over the composition (`ConcreteSystem.bondage_nonempty`, `Systems/Core/System.lean:47`), and a composition of one has no bonded pair. All six bonds are exostructural, and exostructure is structure but never bondage.

State the divergence carefully, because the obvious reading of it is wrong. **It is not that Mobus leaves the box closed and Bunge says to open it.** Mobus opens it two figures later: Fig. 4.15 separates the boundary interfaces, Fig. 4.16 reveals the internal components and the Materials-Purchasing message interface, Fig. 4.17 decomposes Iron-Inventory down to Iron-stock. This entry omits all of that *by design*, at the level the chapter itself pauses on. So both traditions decompose. What they disagree about is **whether the undecomposed view already denotes a system**: for Mobus the SOI at level −1 is one, and its composition is the next question rather than the qualifying one; for Bunge it is a thing until a bonded pair is exhibited. Two traditions putting the moment of systemhood at different points in the same procedure.

Two honesty flags this entry must carry. First, the composition-of-one is **partly our encoding**: Fig. 4.14 has one box, and SL puts the SOI's name on the model with its things one level in, so the single `Combining` component is our construction (the entry's own `note` says this). A different surface syntax might present the same figure as a composition of zero, which Bunge also refuses, but for a different reason. Second, no re-encoding *within SL* escapes the refusal, and inventing a second component to clear it would be the corpus inventing — see the standard below.

> **One caveat this entry must carry.** The Rust gate that fires here is *stronger* than the Lean it was documented as mirroring: `check_bond` additionally requires both endpoints to be `System | Subsystem`, a restriction with no counterpart in `Kernel.HasBond` (see `crates/bert-core/src/validate.rs`). Under the Lean alone, this model satisfies `HasBond`. The paragraph above is therefore written against `Systems/Core/System.lean:47` — Bunge's own definition — rather than against the Rust, so it stands whichever way that discrepancy is resolved. If the extra conjunct turns out to be unjustified, the refusal is our gate's and not Bunge's, and this entry becomes a finding about the kernel instead.

**`klir/cellular-array-cell.sl` is Klir's own lesson, returned by Bunge.** His point in Fig. 4.11 is that the same 5×5 array admits 2²⁵ structure systems, because the environment is placed by the investigator rather than found. The entry encodes his cut, one distinguished cell and four neighbours as environment, and under Bunge that cut yields a composition of one, with eight bonds all exostructural and no system. Move the cut by one cell and the same array becomes a Bunge system with no change to the world. Klir says the observer chooses the system; Bunge's refusal says the observer chooses whether there is one. This refusal is induced by our encoding of his figure rather than forced by his text, which is the honest form of the lesson rather than a weakening of it. The same caveat above applies.

**`bunge/coupling-sigma2.sl` is lossy in sign, and says so — but it is not a refusal.** Bunge distinguishes excitation (+1) from inhibition (−1); SL has no signed relation, and design commitment C3 admits a word only if the kernel already carries the distinction. The polarity survives only in each flow's *name*, where nothing validates it. Recorded as lexicon pressure rather than papered over. It gates clean under all three lenses.

**Two more refusals live outside this directory**, and both matter to how this corpus should be read. `archive/parity-automaton.sl` is Klir-pinned with a self-loop that Mobus refuses for the same §4.3 reason as σ₃. And the two runnable demos, `reservoir` and `allocation`, are both Bunge **aggregates** — a one-process circuit has no bonded pair of components. They stayed invisible because the cross-lens matrix reads `.sl` files and the demos are JSON, so the only three models with dynamics were the only three never cross-checked. Exemptions are where embarrassment hides, and this corpus's standard applies hardest to the flagships. Neither demo should acquire a second component to clear the verdict: the refusal is a true fact about a one-process circuit, and inventing structure to satisfy a gate is the failure this file names below arriving in `demos/`.
