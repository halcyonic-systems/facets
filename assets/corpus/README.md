...
**`mobus/steel-plant.sl` is Mobus's own flagship SL exemplar, and Bunge refuses it.** Fig. 4.14 draws the plant as one opaque box among three sources and three sinks, six flows crossing a boundary and nothing beneath it; the internal components are Fig. 4.16, which this entry omits by design. Under Mobus that is a system at level −1: the boundary is where systemhood is earned, and the composition is the next question. Under Bunge it is not a system at all. Def 1.1's bondage is quantified over the composition (`ConcreteSystem.bondage_nonempty`, `Systems/Core/System.lean:47`), and a composition of one has no bonded pair. All six bonds are exostructural, and exostructure is structure but never bondage. Bunge's answer to Mobus's opening move is: open the box first, a thing with no revealed parts is a thing, not a system. No honest re-encoding escapes this, because Fig. 4.14 has one box and inventing a second would be the corpus inventing. This is the deepest divergence in the library, two traditions locating systemhood in different places.
