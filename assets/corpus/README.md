...
The Mobus ch. 7 set — **Complex → CAS → CAES** — is a sibling set of a different shape, and the difference is worth stating. Klir and Bunge hold the composition fixed and vary the structure. Mobus varies everything: the four models share no component, no flow, and no domain. What is held fixed is the *level* — each is the first pass only, the chapter's own resolution ("we… take the process sufficiently deep to give a good accounting of the first few levels") — and what varies across them is the complexity class. Read in order, the diff is that the environment stops being made of other kinds of thing (computer), starts being made of the SOI's own kind (neuron), acquires a boundary no figure can fix (brain), and finally contains the observer (HSS). Grouping them under `set` stretches the word past "one composition, several structures", and the stretch is recorded here rather than hidden.
