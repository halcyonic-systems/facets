...
| `translation-apparatus.sl` | mobus | **runs** | `ribosome-centers.sl` with the **boundary widened** to bring the aminoacyl-tRNA synthetase inside, so tRNA stops being an input and becomes a carrier that circulates. That one move turns a chain into a loop, and the loop passes gate 3 because `tRNA Pool` is a `Buffering` whose return leg is **matter**. The first *biological* witness for the rule `thermostat.sl` states negatively: the pool is the regulated variable, and the model runs because it was drawn. |
